...
public class GpsCommand implements CommandExecutor, TabCompleter {
    ...
    private void startGps(Player player, Location targetLocation) {
        ...
        boolean roundedDistance = BAirDrop.getInstance().getConfig().getBoolean("airdrop-navigator.rounded-distance", true);
        ...
        BukkitTask task = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (!playerLoc.getWorld().equals(targetWorld)) {
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (args.length == 1) {
            ...
            for (AirDrop air : BAirDrop.airDrops.values()) {
                if (air.isAirDropStarted() && air.getId().toLowerCase().startsWith(args[0].toLowerCase())) {
                    completions.add(air.getId());
                }
            }
            return completions;
        }
