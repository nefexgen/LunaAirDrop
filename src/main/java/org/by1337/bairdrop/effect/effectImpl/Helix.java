        ...
        new BukkitRunnable() {
            ...
            public void run() {
                for (double y = 0; y <= height; y += step) {
                    ...
                    if (particle.name().equals("DUST") || particle.name().equals("REDSTONE"))
                        loc.getWorld().spawnParticle(Particle.DUST, loc.clone().add(offsets).add(x, y, z), count, new org.bukkit.Particle.DustOptions(color, (float) size));
                    else
